...
def POE(POE):
    ...
    if (apikey == ''):
        print (colored('\r\n[x] Unable to execute VTDomainReport - apikey value not input.  Please add one to /opt/mirage/modules/VTDomainReport.py', 'red', attrs=['bold']))
        if (logging == True):
            newlogentry = 'Unable to execute VTDomainReport - apikey value not input.  Please add one to /opt/mirage/modules/VTDomainReport.py'
            LOG.WriteStrongSubLog(POE.logdir, POE.target, newlogentry)
            POE.csv_line += 'N/A,'
        return -1
