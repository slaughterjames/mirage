...
def POE(POE):
    ...
    if (POE.logging == True):
        newlogentry = 'Module: abuse_ch_ransomeware_urls'           
    ...
    if (alx_output_data != ''):                      
        if (POE.logging == True):
            newlogentry = 'Target appears in the abuse.ch URLs feed.'
            LOG.WriteStrongSubLog(POE.logdir, POE.target, newlogentry)
            POE.csv_line += 'True,'
        ...
        if (POE.logging == True):
            newlogentry = 'Target does not appear in the abuse.ch ransomware URLs feed.'
