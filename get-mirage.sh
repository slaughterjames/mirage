...
install_secondary_packages() {
  ...
  #Pull URLHaus URL Blacklist from Abuse.ch and update
  echoinfo "Updating URLHaus Blocklist from Abuse.ch"
  wget -q "https://urlhaus.abuse.ch/downloads/csv_recent/" --output-document "/tmp/URLHaus.txt"
  chmod a+w "/tmp/URLHaus.txt"
  mv "/tmp/URLHaus.txt" "$FEEDS_DIR"
  ...
  #Pull Selenium driver and install 
  echoinfo "Updating Gecko Driver"
  wget -q "https://github.com/mozilla/geckodriver/releases/download/v0.29.0/geckodriver-v0.29.0-linux32.tar.gz" --output-document "/tmp/geckodriver-v0.29.0-linux32.tar.gz"
  tar -xvf "/tmp/geckodriver-v0.29.0-linux32.tar.gz" -C "/tmp/"
  chmod a+w "/tmp/geckodriver"
  mv "/tmp/geckodriver" "/usr/bin/"
  rm "/tmp/geckodriver-v0.29.0-linux32.tar.gz"
...
configure_jarm() {
  #Creates the necessary directories for Jarm in /opt/jarm
  echoinfo "Creating Jarm directories"

  mkdir -p "/opt/jarm" >> $LOGFILE 2>&1
  chmod -R a+w "/opt/jarm" >> $LOGFILE 2>&1

  return 0
}

install_jarm_package() {
  #Pull Jarm from GitHub, unzip and install it
  echoinfo "Installing Jarm"
  wget -q "https://github.com/salesforce/jarm/archive/master.zip" --output-document "/tmp/jarm-master.zip"
  unzip -q "/tmp/jarm-master.zip" -d "/tmp/"
  chmod a+w "/tmp/jarm-master/" 
  mv "/tmp/jarm-master"/* "/opt/jarm" 
  rm -R "/tmp/jarm-master/"
