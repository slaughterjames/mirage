...
class portmap:
    ...
    def Map(self, target, logging, logdir,  debug):
        ...
        for nmap_data in subproc.stdout.readlines():
            ...
            if (nmap_data.find('Host is up') != -1):
                print colored('[*] Host appears to be up.', 'green', attrs=['bold'])
            elif (nmap_data.find('Service Info:') != -1):  
                print colored('[*] ' + nmap_data, 'green', attrs=['bold'])
            elif (nmap_data.find('Host seems down') != -1):
                print colored('[x] Host seems down.', 'red', attrs=['bold'])          
            elif (nmap_data.find('https') != -1):
